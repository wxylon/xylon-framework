 ...
 * {@link Map} interface, making it a member of the
 * <a href="{@docRoot} interface, making it a member of the <a href="{@docRoot}
...
public class Hashtable<K, V> extends Dictionary<K, V> implements Map<K, V>, Cloneable, java.io.Serializable {
	...
	/**
	 * 扩大HashTable的容量, 扩容规则：<b>oldCapacity * 2 + 1</b><br/>
	 * 扩容后桶中的元素不发生变化，桶的索引位置将发生变化
	*/
	...
	/**
	 * value 不能为 <b>null</b><br/>
	 * 若果存在该key则更新,否则新增,新增的时候考虑扩容操作.
	 */
	public synchronized V put(K key, V value) {
		...
		Entry tab[] = table;
		int hash = key.hashCode();
		//hash & 0x7FFFFFFF 取只取hashCode()为1的来算key的位置
		int index = (hash & 0x7FFFFFFF) % tab.length;
		//找到索引，在对应的桶中查找
		for (Entry<K, V> e = tab[index]; e != null; e = e.next) {
			if ((e.hash == hash) && e.key.equals(key)) {
