...
package com.sync.commons.parsing.bytes;

import java.io.UnsupportedEncodingException;

/**
 * Implements a translation buffer for complex byte strings that allows 
 * clients to translate parts of the buffer directly to Unicode and to 
 * substitute other parts with different values. 
 * 
...
public class CharacterTranslationBuffer
{
    ...
    /**
     * Translate pending bytes using selected character set.  Bytes are translated up to
     * current position minus the backoff value. 
     */
    public void translateAndAppendPending(int backoff) throws UnsupportedEncodingException
    {
        ...
        output.append(new String(input, offset + complete,
                len, charset));
        complete = bufferEnd;
    }
